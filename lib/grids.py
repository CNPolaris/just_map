...
def area_to_grid(location, accuracy=10, method='rect', params='auto', form='line'):
    ...
    if method == 'rect':
        temp_points = pd.DataFrame(np.array(
            np.meshgrid(
                np.arange(bounds[0] - delta_lng,
                          bounds[2] + delta_lng,
                          delta_lng / 3),
                np.arange(bounds[1] - delta_lat,
                          bounds[3] + delta_lat,
                          delta_lat / 3)
...
def gps_to_grids_rect(lng, lat, params, from_origin=False):
    """在栅格中找到对应GPS数据的栅格编号
...
def grid_to_params(grid):
    ...
    -------
    params: list or dict
        栅格参数 slon: 起始经度, slat: 起始纬度, delta_lng: 单个栅格经度,
...
# if __name__ == '__main__':
    ...
    # grid, params = area_to_grid(location=bounds, accuracy=10, form='line')
    # grids = grids.set_crs('EPSG:4326', allow_override=True)
    # grids, params = area_to_grid_2(bounds, 10, 'line')
    # lake_path = 'E:\\just\\海韵湖智能技术实验场\\data\\boundary.shp'
    # island_path = 'E:\\just\\海韵湖智能技术实验场\\data\\island.shp'
    #     # grids.plot()
    #     # plt.show()
    #     # grids.to_file('E:\\just\\海韵湖智能技术实验场\\data\\test.shp', driver='ESRI Shapefile', encoding='utf-8')
    # grid_array = gen_grids_array(grid, params, lake_path, island_path, show=True)
    # path = [[1, 1], [1, 2], [1, 3], [1, 4], [1, 5], [1, 6], [1, 7]]
    # gen_path_coord_from_grid(grid, path, params)

#     # obstacle = [(119.3608961, 32.1160124, 10)]
#     # insert_obstacle_into_grids(grids, grid_array, params, obstacle)
